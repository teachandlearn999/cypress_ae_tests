...
describe("AE Tests", () => {
	...
	it("AE_TestCase2", () => {
		...
		cy.get("h2.title.text-center").should(
			"include.text",
			"Account Deleted!"
		);
		cy.get('a[data-qa="continue-button"]').click();
	...
	it("AE_TestCase9", () => {
		...
		cy.get(".features_items .product-image-wrapper").each(
			($product, index) => {
				cy.wrap($product)
					.find(".productinfo p")
					.invoke("text")
				...
				cy.get("button.disabled") // Получаем значение из кнопки <button class="disabled">4</button>
					...
					.then((buttonText) => {
						...
						cy.get("p.cart_total_price") // Проверяем значение в теге <p class="cart_total_price">Rs. 2000</p>
							.invoke("text")
							.then((totalText) => {
								const totalPrice = +totalText.replace(
									"Rs. ",
									""
								); // Преобразуем "Rs. 2000" в число 2000
	...
	it("AE_TC14_ Place Order: Register while Checkout", () => {
		cy.get('div.overlay-content a.add-to-cart[data-product-id="1"]').click({
			force: true,
		});
		cy.get("#cartModal  u").click();
		cy.get('.nav a[href="/view_cart"]').should(
			"have.css",
			"color",
			"rgb(255, 165, 0)"
		);
		...
		cy.get("#address_delivery > :nth-child(4)").should(
			"have.text",
			address1
		);
		...
		cy.get('.nav a[href="/delete_account"]').click();
		cy.get('h2.title.text-center[data-qa="account-deleted"]').should(
			"have.text",
			"Account Deleted!"
		);
	});
	...
	it("AE_TestCase22", () => {
		...
		cy.get("#recommended-item-carousel .active").within(() => {
			cy.get(".add-to-cart")
				.should("be.visible")
				.first()
				.click({ force: true });
		...
		cy.get("#cart_info_table tbody tr")
			.should("be.visible")
			.should("have.length.at.least", 1);
		cy.get('.shop-menu ul.nav li a[href="/view_cart"]').should(
			"have.css",
			"color",
			"rgb(255, 165, 0)"
		);
